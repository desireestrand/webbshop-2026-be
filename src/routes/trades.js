...
tradeRouter.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (
        trade.ownerId._id.toString() !== req.userId &&
        trade.requesterId._id.toString() !==
          req.userId /*&& req.userRole !== "admin"*/
      ) {
      ...
      if (
        trade.requesterId._id.toString() === req.userId &&
        status !== "cancelled"
      ) {
        return res.status(403).json({ message: "Not allowed to update trade" })
      }
